...
public protocol iCloudDelegate {
    ...
    /** Called when the availability of iCloud changes
     ...
     - Parameter cloudIsAvailable: Boolean value that is YES if iCloud is available and NO if iCloud is not available
    ...
    func filesDidChange(_ files: [NSMetadataItem], with filenames: [String])
    ...
    var iCloudQueryLimitedToFileExtension: [String] { get set }
}
